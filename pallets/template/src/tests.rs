...
fn create_trigger_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(TemplateModule::create_triger(Origin::signed(1), timer));
		assert_ok!(TemplateModule::create_triger(Origin::signed(1), schedule));
		assert_ok!(TemplateModule::create_triger(Origin::signed(1), price_gt));
		assert_ok!(TemplateModule::create_triger(Origin::signed(1), price_lt));
	});
}
...
fn create_action_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Dispatch a signed extrinsic.
		assert_ok!(TemplateModule::create_action(Origin::signed(1), mail_with_token));
		assert_ok!(TemplateModule::create_action(Origin::signed(1), oracle));
	});
}

#[test]
fn create_recipe_should_failed() {
	new_test_ext().execute_with(|| {
		let trigger_id = 1;
		let action_id = 1;
		assert_ok!(TemplateModule::create_recipe(Origin::signed(1), trigger_id, action_id));
	});
}
...
fn turn_on_recipe_should_work() {
	new_test_ext().execute_with(|| {
		let recipe_id = 1;
		assert_ok!(TemplateModule::turn_on_recipe(Origin::signed(1), recipe_id));
	});
}
...
fn turn_off_recipe_should_work() {
	new_test_ext().execute_with(|| {
		let recipe_id = 1;
		assert_ok!(TemplateModule::turn_off_recipe(Origin::signed(1), recipe_id));
	});
}

#[test]
fn del_recipe_should_failed() {
	new_test_ext().execute_with(|| {
		let recipe_id = 1;
		assert_ok!(TemplateModule::turn_off_recipe(Origin::signed(1), recipe_id));
	});
